...
function writePreference(key: string, value: string | boolean | number) {
  ...
  window.dispatchEvent(new Event("player-preferences-changed"));
}
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    try {
      const storedVolume = Number(window.localStorage.getItem("jao-ngoh-music-volume") ?? "0.55");
  ...
  useEffect(() => { if (audioRef.current) audioRef.current.volume = musicVolume; }, [musicVolume, playerSettings]);

  function resetGuides() {
    try {
      for (const storage of [window.localStorage, window.sessionStorage]) {
        const keys = Array.from({ length: storage.length }, (_, index) => storage.key(index));
        for (const key of keys) if (key?.startsWith("jao-ngoh-guide-state:") || key?.startsWith("jao-ngoh-guide-seen:")) storage.removeItem(key);
      }
        ...
        <div className="player-settings-layout motion-panel">
          <div className="player-settings-column">
            <section className="player-settings-section" aria-labelledby="guidance-title">
              ...
              <div className="player-settings-row" data-guide="settings-auto-guide"><div><strong>คำแนะนำอัตโนมัติ</strong><p>แสดงคำแนะนำเมื่อคุณพบระบบใหม่ครั้งแรก</p></div><label className="player-toggle"><input aria-label="คำแนะนำอัตโนมัติ" checked={autoGuide} onChange={(event) => { setAutoGuide(event.target.checked); writePreference("jao-ngoh-auto-guide", event.target.checked); }} type="checkbox" /><span aria-hidden="true" /></label></div>
              ...
              <label className="player-settings-subrow"><input checked={showSaveStatus} onChange={(event) => { setShowSaveStatus(event.target.checked); writePreference("jao-ngoh-show-save-status", event.target.checked); }} type="checkbox" /> แสดงสถานะหน้าปัจจุบัน</label>
            </section>
          ...
          <div className="player-settings-column player-settings-column--secondary">
            <section className="player-settings-section" data-guide="settings-sound" aria-labelledby="audio-title"><header><span className="player-eyebrow">AUDIO</span><h2 id="audio-title">เสียง</h2></header><div className="player-settings-row"><div><strong>เสียงตอบสนองของปุ่ม</strong><p>{playerSettings?.soundEffects ? "เปิดหรือปิดเสียงตอบสนองเมื่อกดปุ่ม" : "เสียงตอบสนองยังไม่เปิดให้ใช้งาน"}</p></div><label className="player-toggle"><input aria-label="เสียงตอบสนองของปุ่ม" disabled={!playerSettings?.soundEffects} checked={soundEffects} onChange={(event) => { setSoundEffects(event.target.checked); writePreference("jao-ngoh-sound-effects", event.target.checked); }} type="checkbox" /><span aria-hidden="true" /></label></div>{playerSettings?.music.enabled && playerSettings.music.url ? <div className="player-settings-music"><label className="player-settings-subrow"><input checked={musicEnabled} onChange={(event) => { setMusicEnabled(event.target.checked); writePreference("jao-ngoh-music-enabled", event.target.checked); }} type="checkbox" /> เพลงประกอบ</label><audio ref={audioRef} controls loop muted={!musicEnabled} preload="none" src={playerSettings.music.url} /><input aria-label="ระดับเสียง" max="1" min="0" onChange={(event) => { setMusicVolume(Number(event.target.value)); writePreference("jao-ngoh-music-volume", Number(event.target.value)); }} step="0.05" type="range" value={musicVolume} /></div> : <p className="player-settings-note">เพลงประกอบจะปรากฏเมื่อผู้ดูแลระบบตั้งค่าไฟล์เพลง</p>}</section>
