      ...
      <div className="quantum-interior">
        ...
        <section className="quantum-opening-grid" data-player-reveal="primary">
          <DossierPanel alt={imageAlt} eyebrow={`CASE SCENE / ${subtitle}`} image={image} subtitle="อ่านหลักฐาน ตั้งสมมติฐาน และตัดสินใจด้วยคำอธิบายของคุณเอง" title="เริ่มจากสิ่งที่ข้อมูลบอกคุณ">
            <UtilityStrip><Link href="#timeline">เปิด Timeline</Link><Link href="#evidence">ดูหลักฐาน</Link><Link href={`/submit?subgameId=${subgameId}`}>ส่งเมื่อพร้อม</Link></UtilityStrip>
          </DossierPanel>
          <DecisionPanel action={<Link className="player-button player-button--primary" href="#evidence">เปิดโต๊ะหลักฐาน</Link>} eyebrow="OBJECTIVE / เป้าหมาย" title="คำตอบที่ดีต้องอธิบายได้">
            <p>{description}</p>
        ...
        <section className="quantum-thinking-grid" data-player-reveal="primary">
          <CasePanel eyebrow="CASE BRIEF / วิธีเริ่มคิด" title="อ่านแฟ้มเหมือนนักสืบวิทยาศาสตร์">
        ...
        <DecisionPanel action={<a className="player-button player-button--primary" href="https://gemini.google.com/gem/45cb7e3f0314" rel="noopener noreferrer" target="_blank">เปิด Gemini แล้วกลับมาแนบ PDF</a>} eyebrow="REQUIRED / AI คู่คิด / GEMINI" title="ก่อนส่งคำตอบ ต้องคุยกับ AI คู่คิด">
